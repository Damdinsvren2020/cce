...
class NewsWebTable {
  ...
  static postNews({ title, text, image, language, date }) {
    return new Promise((resolve, reject) => {
      pool.query(
        `INSERT INTO newsweb(title,text,image,language, date) VALUES($1, $2, $3, $4, $5)`,
  ...
  static updateNews({ id, title, text, language, image }) {
    return new Promise((resolve, reject) => {
      pool.query(
        `UPDATE newsweb SET title = $2, text = $3, language = $4, image=$5 WHERE id = $1`,
